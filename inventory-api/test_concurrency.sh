...
# Concurrency test script for the inventory API
# This script demonstrates race conditions in the reserve endpoint

echo "=== Concurrency Test for Inventory API ==="
...
if ! curl -s http://localhost:8000/api/products/1 >/dev/null 2>&1; then
...
# Check initial stock
echo "Initial stock:"
curl -s http://localhost:8000/api/products/1 | jq '.stock_quantity'
...
for i in {1..8}; do
    (
        result=$(curl -s -X POST http://localhost:8000/api/products/1/reserve \
            -H 'Content-Type: application/json' \
            -d '{"quantity":2}')
